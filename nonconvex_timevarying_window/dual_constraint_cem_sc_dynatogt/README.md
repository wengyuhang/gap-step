...
该方法保留原始 SC-DynaTOGT 的 `[K,D]`、SC 映射和 MINCO 结构。当前实现先用 L-BFGS 联合优化原 TOGT 目标与安全软积分，再从该解启动完整协方差 CEM；不含相位模板、共同时间伸缩或任何按窗口形状编写的结构化前端。原始 `sc_dynatogt/` 没有被修改，安全目标封装只位于本方法目录。
...
两个软积分同时用于 L-BFGS 和 CEM 搜索。安全项与 TOGT 动力学项一样，在全部 MINCO 段上按段时长取 8–32 个区间，对每个时间节点直接计算残差与 `smoothedL1` 后做梯形积分；它不先求穿越区间，也不调用碰撞检测。L-BFGS 和 CEM 使用同一 SDF 残差、积分节点、规划半径和固定权重。首次双零后固定再运行 5 轮。返回结果必须另行通过全程 1 ms 动力学密集检测，以及七个窗口全部真实外接球接触区间的 0.2 ms/0.05 ms 安全密集检测；双零候选按飞行时间升序验收，第一条硬检测通过后立即停止。
...
该历史结果早于 2026-09-10 的安全增强 L-BFGS 接入，因此只证明旧的“原 SC 解 + 双约束 CEM”流程；不能冒充当前完整流程的新实验结果。
