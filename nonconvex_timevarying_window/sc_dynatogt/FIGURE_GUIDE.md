...
- 轨迹图里，橙色菱形是规定穿越时刻的轨迹点，它应在当时紫色窗口的真安全区内。
- 紫色窗口不是初始时刻的固定形状，而是轨迹穿过它时的位置、角度和尺寸。
...
当前图里的是 smoke 结果，每个动态组只有 1 次运行，它证明调用链能跑通，不是 30/155 次正式统计结论。右下角的黄色框也特别提醒：原 TOGT 是动力学软惩罚，所以“轨迹优化收敛”不等于“所有时刻都已获得硬约束证书”。
