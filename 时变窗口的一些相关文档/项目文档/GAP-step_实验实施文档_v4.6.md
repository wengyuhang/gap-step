...
本实验实现一个连续二维旋转时变窗口迷宫环境，并训练一个 PPO 特权教师。实验只训练教师，不实现学生模型。教师观测保持低维，包含自身状态、目标相对位置和当前几何射线距离。窗口安全状态只参与当前可碰撞几何构造，不再作为显式窗口列表输入。环境默认奖励只包含目标奖励、碰撞惩罚、时间惩罚和动作惩罚；当前训练配置额外启用连续几何动态进展 shaping，用于缓解稀疏奖励下的超时局部最优。课程推进采用自适应规则：阶段成功率达标后才进入下一阶段，步数上限只作为安全阀。
...
其中 `S` 是当前 episode 实际采样得到的连续迷宫边长。`num_rays` 固定为 32，`ray_max_dist` 随 `S` 缩放，因此测试更大迷宫时感知半径同步扩大。当前 teacher 保留这 39 维作为前缀，并追加时间/窗口特权摘要：

```python
obs_dim = 39 + 2 + max_gate_obs * 12
max_gate_obs = 10
```

### 4.4 时间感知窗口摘要

代码中设置固定 `max_gate_obs = 10`，按距离选取最多 10 个窗口，不足补零。每个窗口摘要 12 维：

```text
present, relative center x/y, distance, normal x/y,
current safe, width clearance, angle error,
time_to_next_safe, time_to_close, current wait cost
```
...
# ray_features 只对当前障碍集合做 raycast，gate_summary 额外提供未来时机信息
obs = np.concatenate([base_39d_prefix, time_sin_cos, gate_summary])
```

因此，测试阶段可以生成不同尺寸、不同窗口数量和不同拓扑复杂度的迷宫，同时保持固定 161 维 teacher 输入。
...
- 每阶段至少训练 `min_steps_per_stage`；
- 最近 `promotion_window_episodes` 个完整 episode 成功率达到 `promotion_success_rate`，且 deterministic train-validation 成功率达到 `promotion_eval_success_rate` 后升阶；
- 达到 soft max 仍未达标时只写入 `soft_max_warning`，不强制升阶；
- 达到 hard max 仍未达标时停止训练，保存当前 checkpoint/metrics，并写入 `hard_max_stop`；
- C5 达标时写入 `completed_success` 并结束训练。
...
其中 `potential(...)` 来自连续几何 visibility roadmap 的 time-dependent Dijkstra 估计。该 roadmap 使用当前连续位置、目标、窗口两侧 approach point 和膨胀墙体转角点；普通边要求线段不穿过膨胀静态障碍，窗口边单独通过窗口两侧 approach point 连接，并从预计到达时间开始采样未来 `gate.is_safe(t)` 计算等待代价。

注意：progress shaping 使用同一 `current_t` 比较 old/new 位置，避免机器人原地等待时仅因窗口未来更接近打开而获得大额正奖励。等待窗口仍可能是策略的一部分，但不作为纯时间流逝的密集奖励。
...
Actor-Critic 网络结构固定为 MLP，动作分布为 tanh-squashed Gaussian：

```text
obs_dim -> 256 -> 256 -> actor_raw_mean(2)
obs_dim -> 256 -> 256 -> value(1)
log_std 为可学习参数，并通过 `min_log_std` / `max_log_std` 限制有效范围
action = tanh(raw_action) * a_max
```

PPO 存储和更新时使用同一个 squashed action 的 log probability，避免旧的 `Normal -> clamp(action)` 路径中 log probability 与实际执行动作不一致。
...
指定 `--stages` 时，对每个阶段运行 `id_test`，用于定位 C1-C5 的失败阶段。

训练指标 `results/train_metrics.csv` 额外记录：
...
progress_delta_mean
progress_delta_abs_mean
dynamic_path_wait_time_mean
dynamic_path_uses_gate_rate
closed_gate_collision_rate
wall_collision_rate
boundary_collision_rate
average_action_norm
