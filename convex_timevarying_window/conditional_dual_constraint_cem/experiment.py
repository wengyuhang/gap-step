...
from convex_timevarying_window.togt.experiment import (
    AUDIT_STEP, BODY_RADIUS, REPO, build_track, dynamic_audit, plot_route,
    safety_audit, scene_record,
)
...
from .objective import SafetyAugmentedTOGTObjective
from .safety_penalty import ConvexSafetyConfig, HandwrittenConvexSafetyIntegral
from .search import ConvexDualCEMConfig, conditional_dual_cem
...
def _manifest():
    ...
    files += sorted((HERE.parent / "togt").glob("*.py"))
    files += sorted((HERE.parent / "togt" / "native").glob("*.cpp"))
    files += [HERE.parent / "geometry.py"]
