...
TOGT 正常收敛，飞行时间 `18.724959468 s`。最新对齐复核直接调用发布版 C++
`QuadManifold::toStateWithTiltYaw`，单旋翼推力峰值为 `5.015527 N > 5 N`，动力学验收失败；
旧结果中的 `5.047529 N` 来自Python平坦性复算，已不再作为当前口径。七窗最小外接球净空依次为
...
相同赛道、相同C++约束配置下的最新基线重跑和新方法对比见
[C++对齐报告](../conditional_dual_constraint_cem/results/formal_cpp_aligned_margin30mm_20260910/REPORT.md)。
