...
def build_track() -> tuple[SCWindowTrack, OptimizationConfig]:
    ...
    config = OptimizationConfig(
        ...
        dynamic_limits=DynamicLimits(
            max_velocity=60.0,
            min_collective_thrust=1.0,
            max_collective_thrust=20.0,
...
def dynamic_audit(trajectory, config: OptimizationConfig, maximum_step=AUDIT_STEP) -> dict[str, Any]:
    ...
    samples = trajectory.sample(samples_per_segment=nodes)
    pvajs = np.stack((
        samples.position, samples.velocity, samples.acceleration,
        samples.jerk, samples.snap,
    ), axis=1)
    native = AnalyticTOGTCore().sample_dynamics(pvajs)
    regular = native["regular_branch"]
    singular = ~regular
    body_rate = native["body_rate"]
    rotors = native["rotor_thrusts"]
    extrema = {
        "max_velocity": float(np.max(native["speed"])),
        "max_tilt": float(np.max(native["tilt"])),
        "max_body_rate_xy": float(np.max(np.linalg.norm(body_rate[:, :2], axis=1))),
        "max_abs_body_rate_z": float(np.max(np.abs(body_rate[regular, 2]))) if np.any(regular) else None,
        "min_collective_thrust": float(np.min(native["collective_thrust"])),
        "max_collective_thrust": float(np.max(native["collective_thrust"])),
        "min_rotor_thrust": np.min(rotors[regular], axis=0) if np.any(regular) else None,
        "max_rotor_thrust": np.max(rotors[regular], axis=0) if np.any(regular) else None,
        "maximum_instantaneous_cpp_penalty": float(np.max(native["instantaneous_penalty"])),
        "regular_branch_samples": int(np.count_nonzero(regular)),
        "singular_branch_samples": int(np.count_nonzero(singular)),
    }
    limits = config.dynamic_limits
    tests = {
        "velocity": extrema["max_velocity"] <= limits.max_velocity + 1e-9,
        "tilt": extrema["max_tilt"] <= TOGT_MAX_TILT + 1e-9,
        "body_rate_xy": extrema["max_body_rate_xy"] <= limits.max_body_rate_xy + 1e-9,
        "body_rate_z": (
            extrema["max_abs_body_rate_z"] is None
            or extrema["max_abs_body_rate_z"] <= limits.max_body_rate_z + 1e-9
        ),
        "collective_thrust": (
        ...
        "rotor_thrust": (
            extrema["min_rotor_thrust"] is None
            or (
    ...
    return {
        ...
        "configuration": {
            ...
            "max_tilt": TOGT_MAX_TILT,
            "max_body_rate_xy": limits.max_body_rate_xy,
            "max_body_rate_z": limits.max_body_rate_z,
            "min_rotor_thrust": limits.min_rotor_thrust,
            "max_rotor_thrust": limits.max_rotor_thrust,
            "min_collective_thrust": limits.min_collective_thrust,
            "max_collective_thrust": limits.max_collective_thrust,
            "weights": {
                "velocity": 0.0, "body_rate": 1.0,
                "rotation": 1.0, "thrust": 1.0,
            },
        },
        "backend": "released TOGT C++ QuadManifold::toStateWithTiltYaw",
        "evidence": "full-flight dense sampling with released TOGT C++ dynamics; not a continuous certificate",
    }
